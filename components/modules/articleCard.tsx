...
function ArticleCard({ article }: { article: ArticleT }) {
  return (
    <Link href={`/articles/${article.slug}`} className="card card-hover flex flex-col group hover:bg-transparent border-2 border-transparent transition-all hover:border-primary shadow-2xl shadow-transparent hover:shadow-primary/30">
      <Image
        width={373}
        height={210}
        alt={article.title}
