...
function Home({ carousel, categories, newestProducts, otherProducts, specialProducts, banners }: { carousel: PageSectionItemT[]; newestProducts: PageSectionT | undefined; otherProducts: PageSectionT | undefined; categories: CategoryT[]; specialProducts: PageSectionT | undefined; banners: PageSectionItemT[]; }) {
  return (
    <>
      ...
      <div className="wrapper mt-12 lg:mt-24">
        <div className="flex items-center gap-3">
          <h2 className="heading">{newestProducts?.title}</h2>
        ...
        <Carousel
          ...
          className="mt-6"
        >
          {newestProducts?.items.map((product) => (
            <SwiperSlide key={product.id} className="pb-16">
              <ProductCard product={mapSectionItemToProduct(product.values[0])} />
            </SwiperSlide>
          ))}
        </Carousel>
      </div>
          ...
          >
            {specialProducts?.items.map((product) => (
              <SwiperSlide key={product.id} className="pb-16">
                <ProductCard
                  isSpecial
                  product={mapSectionItemToProduct(product.values[0])}
                />
              </SwiperSlide>
            ))}
          </Carousel>
      ...
      <div className="wrapper mt-24 lg:mt-40">
        <div className="flex items-center gap-3">
          <h4 className="heading">{otherProducts?.title}</h4>
          <Link href={"/products"} className="mr-auto">
            <Button variant="outline">همه</Button>
          </Link>
        </div>
        ...
        <Carousel
          ...
          className="mt-6"
        >
          {otherProducts?.items.map((product) => (
            <SwiperSlide key={product.id} className="pb-16">
              <ProductCard product={mapSectionItemToProduct(product.values[0])} />
            </SwiperSlide>
          ))}
