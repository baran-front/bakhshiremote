...
type GetProfileDateByRepresentativeDateBrifParamsT = {
  fromDate: string;
  toDate: string;
  token: string;
};
export const getProfileDateByRepresentativeDateBrif = ({
  token,
  fromDate,
  toDate,
}: GetProfileDateByRepresentativeDateBrifParamsT) =>
  safeFetch<{
    data?: unknown;
  }>(
    `/v1/personal/profiledatebyrepresentativedatebrif?fromDate=${encodeURIComponent(
      fromDate
...
export const getCategoryDetail = (params: GetCategoryDetailParamsT) =>
  safeFetch<{ data: CategoryT }>(`/v1/categories/${params.id}`, {});
